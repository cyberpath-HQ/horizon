...
export default function DashboardPage() {
    ...
    const {
        data: healthData, isLoading: healthLoading, error: healthError,
    } = useHealth();
...
: null;
    ...
    // Determine if health banner should be shown (not healthy or loading/error)
    ...
    return (
        <div className="space-y-6">
            {/* Health Status Banner - Only show when not healthy or loading/error */}
            {showHealthBanner && (
                <Card className={cn(healthError ? `border-destructive` : health?.status === `healthy` ? `border-green-500` : `border-yellow-500`)}>
...
? (
                                            <span className="text-destructive flex items-center gap-1">
                                                ...
                                                {healthError instanceof Error ? healthError.message : `Unable to connect to server`}
                                            </span>
                                          )
...
: (
                            ...
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => window.location.reload()} disabled={healthLoading}>
            ...
            {/* Quick Actions */}
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="hover-lift cursor-pointer" onClick={async() => navigate({
                    to: `/dashboard/agents`,
                })}>
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">
